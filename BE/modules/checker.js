...
async function waitForPin(username) {
  console.log(
    chalk.blue(`🛑 Waiting for the PIN for [${username}] from user response...`)
  );
...
function receivePin(pin) {
  if (pendingPinResolver) {
    pendingPinResolver(pin.trim());
    pendingPinResolver = null;
  } else {
    console.error("❗ No PIN request pending.");
  }
}
...
// Export fungsi checkAccount agar dapat digunakan di route
module.exports = { checkAccount, startCheckingInternal };
