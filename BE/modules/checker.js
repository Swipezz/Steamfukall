...
let pendingPinResolvers = {}; // Objek untuk menyimpan resolve functions berdasarkan username

async function waitForPin(username) {
    console.log(chalk.blue(`🛑 Waiting for the PIN for [${username}] from API...`));
...
// Fungsi ini akan dipanggil oleh endpoint API untuk mengirimkan PIN
function receivePinFromApi(username, pin) {
    ...
    if (pendingPinResolvers[username]) {
        ...
        // resolve(pendingPinResolvers[username])
    } else {
    ...
    try {
        ...
        if (loginData.success) {
            ...
            await (async () => {
                ...
                // await waitForPin(user); // Tunggu PIN dari API
                // PIN akan diisi secara otomatis oleh receivePinFromApi
                console.log(chalk.yellow(`⏳ Waiting for PIN input from API for [${user}]...`));
                // Tidak perlu sleep di sini, promise waitForPin akan resolve saat PIN diterima

                // Asumsi steamLogin.submitPin dapat diakses dan memerlukan page dan pin
                const pinFromApi = await waitForPin(user); // Dapatkan PIN setelah di-resolve
...
// Export fungsi checkAccount dan receivePinFromApi
